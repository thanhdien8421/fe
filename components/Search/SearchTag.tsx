...
const SearchTag = () => {
  ...
  return (
    ...
    <div className="relative bg-white">
      ...
      <div className={`flex rounded-sm w-auto absolute ${open ? "max-h-[300px] min-h-[300px] p-6 border bg-white border-gray-600" : "max-h-0"} `} 
      >
        <div className="overflow-y-auto  max-h-[300px]">
          {career_directory.map((career, index) => {
            return (
              <li key={career.occupational_group} className="flex justify-between mt-1 mr-2 mb-6 max-w-[300px] "
        ...
        <div className="overflow-y-auto  max-h-[300px]">
          {activeCategory !== null && (career_directory[activeCategory].profession.map((profession: any) => {
            return (
              <Specialization key={profession.name} profession={profession} />
            )
          }))}
        </div>
      </div>
    ...
    <div className="flex flex-row border-b border-gray-300 bg-white">
      ...
      <div className=" max-w-[500px] bg-white">
        {profession.specializes.map((tag: any) => {
          return (
            <button className="text-gray-700 bg-slate-300 m-2 text-center rounded-[15px] text-[15px] w-max pt-1 pb-1 pl-2 pr-1"
            key = {tag}>{tag}</button>
          )
        })}
      </div>
