...
export default function Education({type, getData} : {type : string, getData : RecordType}) {
  const [data, setData] = React.useState<EducationAttr[]>([]);
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        `http://localhost:8000/api/v1/educations/employee/1`
      );
      if (!response.ok) {
    ...
    <Card className="drop-shadow-sm h-[400px] pb-2">
      ...
      <CardContent className="my-5 gap-1 h-3/4">
        <div className="flex flex-col overflow-y-auto gap-4 h-full">
          {data.map((obj) => {
            return (
              <EducationCard obj={obj} type={type} getData={getData} key={obj.id} />
            )
          }
          )}
        </div>
...
export const EducationCard: React.FC<EducationProps> = ({ obj, type, getData}) => {
  return (
    <div className="flex flex-row w-full gap-10 p-3 bg-white border border-gray-200 rounded-lg shadow-md transition duration-300 hover:shadow-xl hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
      <div className="flex flex-row justify-between gap-10">
        <div className="grid grid-cols-5 gap-3 justify-between">
          ...
          ) : ( type == "preview" ? <></> : <Checkbox onClick={() => {getData.education.push(obj.id)} }/>)
          }
      </div>
    </div>
  )
